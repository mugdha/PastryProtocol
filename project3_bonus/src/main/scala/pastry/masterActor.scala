...
class masterActor(numNodes: Int, numReq: Int, mySys: ActorSystem) extends Actor {
  ...
  def receive = {
    ...
    case Joined(nodeId: String) =>
      ...
      if(count == numNodes) {
        /*
         * Kill a random node here
         */
        var peer = peerList(Random.nextInt(numNodes))
        println("Killing node - "+peer.path.toString().substring(19))
        val roundScheduler = context.system.scheduler.scheduleOnce(0 seconds, peer, PoisonPill)
        /*
         * Start routing
         */
        Thread.sleep(1000)
     ...
     if (terminateCount >= ((numNodes-1) * numReq)) {
       Thread.sleep(1000)
       println("All nodes have finished routing ...\n")
       println("================================================================================================================================")
       println("TOTAL ROUTES = " + ((numN-1) * numR))
       println("TOTAL HOPS = " + totalHops)
       println("AVERAGE HOPS PER ROUTE = " + totalHops / ((numN-1) * numR))
       println("================================================================================================================================")
